...
float Properties:: cal_distance(Point3D const &p1, Point3D const &p2){
...
float Properties:: cal_area(Point3D const &p1, Point3D const &p2, Point3D const &p3){
...
std::string Properties :: outputArea(float area){
    std::string ans =  "Area of 3D object is ";
    ans += std::to_string( area );
    return ans;
}
...
std::string Properties:: get_surface_area()
{
    float area{};
    for(Triangle triangle : triangles){
        area += cal_area(triangle.p1(), triangle.p2(), triangle.p3());
    }
    return outputArea(area);
}